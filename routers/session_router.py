...
class SessionEndpoint:
    ...
    def login(self, login_req: LoginRequest):
        with SqlAlchemyDatabaseManager.session("primary") as session:
            ...
            token = auth_service.create_token(user.login)
            privileges = PrivilegeService(session).find_by_user_login(user.login)
    ...
    def refresh(self, refresh_req: RefreshRequest):
        with SqlAlchemyDatabaseManager.session("primary") as session:
            ...
            new_token = auth_service.create_token(old_token.user)
            auth_service.delete(old_token)
            privileges = PrivilegeService(session).find_by_user_login(new_token.user)

            return TokenResponse(
                token=new_token.code,
                expires_at=new_token.expires,
                identity=new_token.user,
                privileges=privileges,
            )
